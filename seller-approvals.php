...
function getSellerDocuments($seller_id) {
    ...
    $query = "SELECT * FROM seller_documents WHERE seller_id = $seller_id AND document_type = 'Valid ID'";
    $result = $db->query($query);
    ...
    foreach ($documents as &$doc) {
        ...
        $doc['document_url'] = $file_path;
        $doc['file_exists'] = file_exists($file_path) && is_readable($file_path);
    }
...
function verifySellerDocument($document_id, $is_verified = true) {
    global $db;
    $document_id = intval($document_id);
    
    $query = "UPDATE seller_documents SET is_verified = ? WHERE id = ?";
    $stmt = $db->prepare($query);
    $stmt->bind_param("ii", $is_verified, $document_id);
    return $stmt->execute();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && isset($_POST['seller_id'])) {
    ...
    try {
        if ($action == 'approve') {
            // Update seller status to approved - using a simpler query to avoid column issues
            $stmt = $db->prepare("UPDATE sellers SET status = 'approved', approved_at = NOW() WHERE id = ?");
            $stmt->bind_param("i", $seller_id);
            $stmt->execute();
            
            // Update admin_notes and approved_by separately to avoid errors if columns don't exist
            if (!empty($admin_notes)) {
                $stmt = $db->prepare("UPDATE sellers SET admin_notes = ? WHERE id = ?");
                $stmt->bind_param("si", $admin_notes, $seller_id);
                $stmt->execute();
            }
            
            // Try to update approved_by if the column exists
            $stmt = $db->prepare("UPDATE sellers SET approved_by = ? WHERE id = ?");
            ...
            // Update seller status to rejected - using a simpler query to avoid column issues
            $stmt = $db->prepare("UPDATE sellers SET status = 'rejected', rejected_at = NOW() WHERE id = ?");
            $stmt->bind_param("i", $seller_id);
            $stmt->execute();
            
            // Update admin_notes and rejected_by separately to avoid errors if columns don't exist
            if (!empty($admin_notes)) {
                $stmt = $db->prepare("UPDATE sellers SET admin_notes = ? WHERE id = ?");
                $stmt->bind_param("si", $admin_notes, $seller_id);
                $stmt->execute();
            ...
            // Try to update rejected_by if the column exists
            $stmt = $db->prepare("UPDATE sellers SET rejected_by = ? WHERE id = ?");
            $stmt->bind_param("ii", $_SESSION['user_id'], $seller_id);
            $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['verify_document'])) {
    ...
    try {
        if (verifySellerDocument($document_id, $is_verified)) {
            echo json_encode(['success' => true, 'message' => 'Document verification status updated.']);
        } else {
...
<div class="container mx-auto px-4">
    <h2 class="text-2xl font-bold mb-6">Seller Approvals</h2>
    ...
    <?php if ($seller_details): ?>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="?page=seller-approvals<?php echo isset($_GET['status']) ? '&status=' . $_GET['status'] : ''; ?>" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-2">
                    <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <p class="font-medium">
                                    <?php if (isset($seller_details['website']) && $seller_details['website']): ?>
                        ...
                        <?php if (isset($seller_details['description']) && $seller_details['description']): ?>
                            <div class="mt-4">
                                    ...
                                    <div class="border border-gray-200 rounded-lg p-4">
                                        <div class="flex justify-between items-center mb-2">
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-500">Uploaded: <?php echo date('M d, Y h:i A', strtotime($document['uploaded_at'])); ?></p>
                                            </div>
                                        ...
                                        <?php if ($document['file_exists'] && $is_image): ?>
                                            <div class="mb-3 border border-gray-300 rounded-lg p-2 bg-white">
                                                <img src="<?php echo htmlspecialchars($document['document_url']); ?>" 
                                                     alt="<?php echo htmlspecialchars($document['document_type']); ?>" 
                                                     class="max-w-full h-auto max-h-96 object-contain mx-auto"
                                                     onerror="this.parentElement.innerHTML='<div class=\"text-center text-red-600 py-4\"><i class=\"fas fa-image text-4xl mb-2\"></i><p>Unable to load image</p></div>'">
                                            </div>
                                        <?php elseif ($document['file_exists'] && !$is_image): ?>
                                            <div class="bg-gray-100 p-4 rounded flex items-center justify-center mb-3">
                                                <i class="fas fa-file-pdf text-red-500 text-4xl mr-3"></i>
                                                <div>
                                                    <p class="font-medium">PDF Document</p>
                                                    <p class="text-sm text-gray-500">Click "View" to open</p>
                                                </div>
                                            </div>
                                        <?php else: ?>
                                            <div class="bg-red-50 p-4 rounded flex items-center justify-center mb-3 border border-red-200">
                                                <div class="text-center">
                                                    <i class="fas fa-exclamation-triangle text-red-500 text-3xl mb-2"></i>
                                                    <p class="font-medium text-red-800">File Not Found</p>
                                                    <p class="text-sm text-red-600">The uploaded file could not be located on the server.</p>
                                                    <p class="text-xs text-red-500 mt-1">Path: <?php echo htmlspecialchars($document['document_url']); ?></p>
                                                </div>
                                            </div>
                                        <?php endif; ?>
                                        ...
                                        <div class="flex gap-2">
                                            <?php if (!$document['is_verified']): ?>
                                                <button type="button" onclick="verifyDocument(<?php echo $document['id']; ?>, true)" class="flex-1 px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded text-sm font-medium">
                                                    <i class="fas fa-check mr-1"></i> Verify ID
                                                </button>
                                                <button type="button" onclick="verifyDocument(<?php echo $document['id']; ?>, false)" class="flex-1 px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded text-sm font-medium">
                                                    <i class="fas fa-times mr-1"></i> Reject ID
                                                </button>
                                            <?php else: ?>
                                                <button type="button" onclick="verifyDocument(<?php echo $document['id']; ?>, false)" class="flex-1 px-3 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded text-sm font-medium">
                                                    <i class="fas fa-times mr-1"></i> Unverify ID
                                                </button>
                                            <?php endif; ?>
                                        </div>
                                        <?php endif; ?>
                    ...
                    <?php else: ?>
                        <div class="bg-yellow-50 rounded-lg p-6 border border-yellow-200 mt-6">
                            <div class="flex items-start">
                                <div class="flex-shrink-0">
                                    <i class="fas fa-exclamation-triangle text-yellow-600 text-xl"></i>
                                </div>
                                <div class="ml-3">
                                    <h3 class="text-sm font-medium text-yellow-800">No Valid ID Uploaded</h3>
                                    <p class="text-sm text-yellow-700 mt-1">The seller has not uploaded a valid ID yet. They cannot post products until this is verified.</p>
                                </div>
                            </div>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="bg-gray-50 rounded-lg p-6 border border-gray-200 sticky top-6">
                        ...
                        <div class="mb-4">
                            ...
                            <?php elseif ($seller_details['status'] == 'approved'): ?>
                                ...
                                <?php if (isset($seller_details['approved_at']) && $seller_details['approved_at']): ?>
                            ...
                            <?php elseif ($seller_details['status'] == 'rejected'): ?>
                                ...
                                <?php if (isset($seller_details['rejected_at']) && $seller_details['rejected_at']): ?>
                                <p class="text-sm text-gray-500 mt-2">
                        ...
                        <?php if (isset($seller_details['admin_notes']) && $seller_details['admin_notes']): ?>
                            <div class="mb-4">
                        ...
                        <?php if ($seller_details['status'] == 'approved' || $seller_details['status'] == 'rejected'): ?>
                            <div class="mt-4">
                                <a href="?page=seller-approvals&status=pending" class="block w-full py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md text-center">
    ...
    <?php else: ?>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800">Seller Approvals</h2>
            ...
            <div class="flex flex-wrap mb-4">
                <a href="?page=seller-approvals&status=pending&search=<?php echo urlencode($search); ?>" 
                ...
                <a href="?page=seller-approvals&status=approved&search=<?php echo urlencode($search); ?>" 
                ...
                <a href="?page=seller-approvals&status=rejected&search=<?php echo urlencode($search); ?>" 
                ...
                <a href="?page=seller-approvals&status=all&search=<?php echo urlencode($search); ?>" 
            ...
            <form action="" method="GET" class="mb-6">
                <input type="hidden" name="page" value="seller-approvals">
                ...
                <div class="flex gap-2">
                    ...
                    <?php if (!empty($search)): ?>
                    <a href="?page=seller-approvals&status=<?php echo htmlspecialchars($status_filter); ?>" class="bg-gray-300 text-gray-700 px-4 py-2 rounded hover:bg-gray-400">
                        Clear
                    </a>
                    <?php endif; ?>
                </div>
                            ...
                            <?php foreach ($sellers as $seller): ?>
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap border-b border-gray-200 text-sm font-medium text-right">
                                        <a href="?page=seller-approvals&view=<?php echo $seller['id']; ?><?php echo isset($_GET['status']) ? '&status=' . $_GET['status'] : ''; ?>" class="text-blue-600 hover:text-blue-900 mr-3">
                                            <i class="fas fa-eye mr-1"></i> View
                                        </a>
                                        <?php if ($seller['status'] == 'pending'): ?>
                                            <a href="?page=seller-approvals&view=<?php echo $seller['id']; ?>" class="text-green-600 hover:text-green-900 mr-3">
                    ...
                    <div class="mt-4 flex justify-center">
                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                            <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                                <a href="?page=seller-approvals&p=<?php echo $i; ?>&status=<?php echo urlencode($status_filter); ?>&search=<?php echo urlencode($search); ?>" 
...
<script>
    ...
    function verifyDocument(documentId, isVerified) {
        const action = isVerified ? 'verify' : 'reject';
        const confirmMsg = isVerified ? 'Are you sure you want to verify this document?' : 'Are you sure you want to reject this document?';
        ...
        fetch('', {
            ...
            body: `verify_document=1&document_id=${documentId}&is_verified=${isVerified ? 1 : 0}`
        })
